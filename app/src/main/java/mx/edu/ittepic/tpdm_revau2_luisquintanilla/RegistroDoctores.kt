...
class RegistroDoctores : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnInserta?.setOnClickListener {
            val ventanaTarea = Intent(this,DoctorInsertar::class.java)
            startActivity(ventanaTarea)

        }
        btnEditar?.setOnClickListener {
            val ventanaTarea = Intent(this,DoctorEditar::class.java)
            startActivity(ventanaTarea)
        }
                ...
                if (cur.moveToFirst() == true) {
                    do{
                        dato +="ID: ${cur.getString(0)}\nNombre: ${cur.getString(1)}\nApellido: ${cur.getString(2)}\nEdad:${cur.getString(3)}\n "+
                                "Especialidad:${cur.getString(4)}\n Telefono:${cur.getString(5)}\n "
    ...
    fun buscar(id: String, btnEtiqueta: String){
        try {
            ...
            var con="SELECT * FROM DOCTOR WHERE IDDOCTOR="+id
            var  cur = trans.rawQuery(con,null)

            if (cur.moveToFirst()==true){
                if (btnEtiqueta.startsWith("e")){
                    var dato = "¿Seguro de eliminar al Doctor: \"${cur.getString(1)}\" con el ID \"${cur.getString(0)}\" ?"
    ...
    fun eliminar(id:String){
        try{
            ...
            var SQL = "DELETE FROM DOCTOR WHERE IDDOCTOR="+id
            trans.execSQL(SQL)
            trans.close()
