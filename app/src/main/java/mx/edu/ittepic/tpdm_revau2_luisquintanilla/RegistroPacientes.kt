...
class RegistroPacientes : AppCompatActivity() {
        ...
        try {
            ...
            if (cur.moveToFirst()==true){
                if (btnEtiqueta.startsWith("e")){
                    var dato = "¿Seguro de eliminar al Paciente: \"${cur.getString(1)}\" con el ID \"${cur.getString(0)}\" ?"
    ...
    fun eliminar(id:String){
        try{
            ...
            var SQL = "DELETE FROM PACIENTE WHERE IDPACIENTE="+id
            trans.execSQL(SQL)
            trans.close()
